...
def getRMatrix(yi, yf):
    ...
    if (abs(phi) > 0.1):
        ...
        print("np.sin(phi) ================= ", np.sin(phi))
        print("np.sin(phi)[0] ================= ", np.sin(phi)[0])
        print("\n\n")
        # s_hat = np.array([[0, -ax[2], ax[1]], [ax[2], 0, -ax[0]], [-ax[1], ax[0], 0]])
        s_hat = np.array([[0, -ax[2][0], ax[1][0]], [ax[2][0], 0, -ax[0][0]], [-ax[1][0], ax[0][0], 0]])
        # R = np.eye(3) + np.sin(phi) * s_hat + (1 - np.cos(phi)) * np.dot(s_hat, s_hat)      # dot???
        R = np.eye(3) + np.sin(phi)[0] * s_hat + (1 - np.cos(phi)[0]) * np.dot(s_hat, s_hat)      # dot???
        print("R ====================== ", R)
    else:
        R = np.eye(3)
    return R
